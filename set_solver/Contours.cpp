...
void Contours::filter_contours_near_edges(int distance_threshold) {
    int top_edge_threshold = distance_threshold;
    int bottom_edge_threshold = rows - distance_threshold;
    int left_edge_threshold = distance_threshold;
    int right_edge_threshold = cols - distance_threshold;
    ...
    for (auto i = 0; i < contours.size(); i++) {
        ...
        for (cv::Point const &point : contours[i]) {
